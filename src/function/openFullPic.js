    ...
    return (
        <>
            <div className="modall" >
                <button className="close-img" onClick={() => props.handleFullImg(null)}><Icon.X /></button>
                {/* <img className={zoomIn ? 'zoom-in' : 'zoom-out'} id="img" src={modalSrc} onClick={handleZoom} /> */}
                {!onL &&
                    <Space direction="vertical" style={{ height:'100%', width:'100%',position:'fixed',top:'50%'}}>
                        <Spin tip="Loading" size="large" >
                            <div className="content" />
                        </Spin>

                    </Space>}
                <img className={zoomIn ? 'zoom-in' : 'zoom-out'} id="img" src={modalSrc} onClick={handleZoom} onLoad={han}/>
            </div >
        </>)
